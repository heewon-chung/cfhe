...
int main(){
    ...
    ZZ Msg1, Msg2;
    vector<ZZX> message1, message2, subResult, signResult;
    
    Ctxt ct1(publicKey), ct2(publicKey);
    Ctxt subCt(publicKey);
    ...
    subtract(subCt, ct1, ct2, currentLength, ea);
    ea.decrypt(subCt, secretKey, subResult);

    cout << endl;
    cout << "Add Result (Plain): " << (Msg1 - Msg2) << endl;
    cout << "Add Result (Encrypted): " << vector2Long(subResult, currentLength) << endl;
    cout << "Add Levels Left: " << subCt.findBaseLevel() << endl;
